class Tesla:
    ...
    def autopilot(self, obsticle: str) -> str:
        """ Checks autopilot and obstacle is present, returns a string"""
        if self.__autopilot:
    ...
    def drive(self, travel_range: float) -> str:    
        """
        Returns the battery charge level based on battery discharge percent

            Parameters:
                    travel_range(float): A decimal integer
            
            Returns:
                    battery_charge(str): Another decimal integer
        """
